...
build {
  ...
  provisioner "shell" {
    environment_vars = ["STAGING_DIR=${var.staging_dir}"]
    execute_command  = "echo ${var.ssh_password} | {{ .Vars }} sudo -S -E bash '{{ .Path }}'"
    scripts          = ["scripts/oracle19c/preinstall_oracle19c.sh"]
  }

  provisioner "file" {
    destination = "${var.staging_dir}"
  ...
  provisioner "shell" {
    environment_vars = ["STAGING_DIR=${var.staging_dir}"]
    execute_command  = "echo ${var.ssh_password} | {{ .Vars }} sudo -S -E bash '{{ .Path }}'"
    scripts          = ["scripts/oracle19c/install_oracle19c.sh", "scripts/basebox/cleanup.sh", "scripts/basebox/zerodisk.sh"]
  }
